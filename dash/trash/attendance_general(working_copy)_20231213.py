        ...
        html.Div(
            children=[
                html.Div(
                    children=[
                        html.H2(
                            "Aanwezigheidsgegevens van parlementsleden per party",
                            className="header-subtitle",
                        ),
                        html.P(
                            children=(
...
def update_attendance_per_party(commissions_overview_df_input, fracties_dict_input):
    ...
    attendance_per_party_percentage_formatted = attendance_per_party_percentages.applymap(lambda x: f'{x:.2f}')
...
def update_attendance_per_party_graph(attendance_per_party_percentage):
    ...
    trace_aanwezig = go.Bar(y=sorted_parties, x=bars_aanwezig, orientation='h', name='aanwezig')
    trace_afwezig = go.Bar(y=sorted_parties, x=bars_afwezig, orientation='h', name='afwezig')
    trace_verontschuldigd = go.Bar(y=sorted_parties, x=bars_verontschuldigd, orientation='h', name='verontschuldigd')
    ...
    layout = go.Layout(
        barmode='stack',
        title='Aanwezigheid verschillende partijen',
        xaxis=dict(title='Aanwezigheid'),
